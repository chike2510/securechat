...
const encryptedAttachmentSchema = z.object({ id: z.number().int().positive(), name: z.string().min(1).max(120), mediaType: z.string().min(1).max(120), size: z.number().int().positive().max(3 * 1024 * 1024), ciphertextPath: z.string().min(1).max(200), iv: z.string().min(1).max(128), kind: z.enum(["file", "voice"]), waveform: z.array(z.number().min(0).max(1)).max(80).optional() });
...
export const appRouter = router({
  ...
  secureChat: router({
    ...
    friendProfile: universityProcedure.input(z.object({ userId: z.number().int().positive() })).query(({ ctx, input }) => getFriendProfile(ctx.user.id, input.userId)),
    profileSettings: universityProcedure.query(({ ctx }) => getProfileSettings(ctx.user.id)),
    updateProfile: universityProcedure.input(z.object({ name: z.string().trim().min(2).max(60), username: z.string().trim().min(3).max(24).regex(/^[a-zA-Z0-9_.]+$/).optional(), avatarStyle: z.enum(["ink", "mint", "rose", "violet"]), imageData: z.string().max(700_000).nullable().optional(), imageType: z.enum(["image/jpeg", "image/png", "image/webp"]).nullable().optional(), clearImage: z.boolean().optional() })).mutation(({ ctx, input }) => updateProfileSettings(ctx.user.id, input)),
    ...
    openConversation: universityProcedure.input(z.object({ userId: z.number().int().positive() })).mutation(({ ctx, input }) => getAcceptedDirectConversation(ctx.user.id, input.userId)),
