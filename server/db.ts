...
import {
  ...
  notificationFor,
} from "../shared/message-lifecycle.js";
...
type ConversationInfo = {
  ...
  groupKeyEnvelopes: Record<
    string,
    { ciphertext: string; iv: string; ownerPublicKey: string }
  >;
  groupKeyEnvelopesByVersion: Record<
    string,
    Record<string, { ciphertext: string; iv: string; ownerPublicKey: string }>
  >;
  createdAt: string;
  updatedAt: string;
};
...
type StoredDeviceLinkSession = {
  sessionId: string;
  ownerSubject: string;
  recipientSubject: string | null;
  ownerPublicKey: string;
  recipientPublicKey: string | null;
  bundle: StoredDeviceLinkBundle | null;
  createdAt: string;
  expiresAt: string;
  usedAt: string | null;
};
...
let _readiness: "unconfigured" | "initializing" | "ready" | "failed" =
  "unconfigured";
let _failureCategory:
  | "authentication"
  | "connection"
  | "schema"
  | "unknown"
...
export function getSupabaseStorageConfiguration(
  ...
  if (!_initialization) {
    _initialization = (async () => {
      ...
      try {
        ...
        console.error(
          "[SecureChatStorage] private storage initialization failed",
...
export async function getDatabaseReadiness() {
  ...
  return {
    configured:
      configuration.urlConfigured && configuration.serverKeyConfigured,
...
function numericId(subject: string) {
  const hash = createHash("sha256")
    .update(`securechat:user:${subject}`)
    .digest();
...
function directConversationId(firstSubject: string, secondSubject: string) {
  ...
  const hash = createHash("sha256")
    .update(`securechat:conversation:${pair}`)
    .digest();
  return Math.max(1, hash.readUInt32BE(0) & 0x7fffffff);
}
...
function deviceLinkPath(sessionId: string) {
  return `device-links/${sessionId}.json`;
}
...
async function listJson<T>(prefix: string): Promise<T[]> {
  const store = await getStore();
  if (!store) return [];
  const { data, error } = await store.storage.from(BUCKET).list(prefix, {
    limit: 100,
    offset: 0,
  ...
  const items = await Promise.all(
    files.map(item => readJson<T>(`${prefix}/${item.name}`))
  );
...
function metadataString(
  ...
  key: "name" | "username" | "matricNumber"
) {
...
function profileFromAuthUser(
  user: SupabaseAuthUser,
  previous?: RemoteProfile
): RemoteProfile | undefined {
  const matricNumber =
    previous?.matricNumber ??
    metadataString(user, "matricNumber")?.toUpperCase();
  ...
  return {
    ...
    username:
      previous?.username ??
      normalizedUsername(metadataString(user, "username"), user.id),
...
function profileToUser(profile: RemoteProfile): User {
  return {
    ...
    name: profile.name,
    username:
      profile.username ?? normalizedUsername(profile.name, profile.subject),
    email: profile.email,
...
async function listAuthUsers() {
  const store = await getStore();
  if (!store) return [];
  const { data, error } = await store.auth.admin.listUsers({
    page: 1,
    perPage: 1000,
  });
  if (error) throw error;
...
export async function upsertUser(user: InsertUser): Promise<void> {
  if (!user.openId || !user.matricNumber)
    throw new Error("SecureChat user identity is required");
  await ensureProfile({
    openId: user.openId,
...
export async function getUserByOpenId(openId: string) {
  const profile = await readJson<RemoteProfile>(
    profilePath(subjectFromOpenId(openId))
  );
...
async function getUserByOpenIdWithImage(openId: string) {
  ...
  return {
    ...
    profileImageUrl: await profileImageUrl(profile),
  };
}

export function selectSupabaseProfile<T>(
  byOpenId: T | undefined,
  byMatricNumber: T | undefined,
  byEmail: T | undefined
) {
...
export async function getUserByMatricNumber(matricNumber: string) {
  ...
  const authUser = users.find(
    user => metadataString(user, "matricNumber")?.toUpperCase() === normalized
  );
...
export async function createLocalUser() {
  throw new Error(
    "Local password profiles are not supported; SecureChat uses Supabase Auth."
  );
}
...
async function latestProfileImagePath(store: SupabaseClient, subject: string) {
  ...
  return selectLatestProfileImagePath(
    subject,
...
async function profileImageUrl(profile: RemoteProfile | undefined) {
  ...
  for (const path of paths) {
    const result = await store.storage
      .from(BUCKET)
...
export async function searchUsers(currentUserId: number, query: string) {
  ...
  const results: Array<User | undefined> = await Promise.all(
    authUsers.slice(0, 1000).map(async authUser => {
      const stored = await readJson<RemoteProfile>(profilePath(authUser.id));
      const profile = stored ?? profileFromAuthUser(authUser);
      return profile ? profileToUser(profile) : undefined;
    })
  ...
  const profiles = results.filter(
    (profile): profile is User => profile !== undefined
  );
  const discoverableProfiles = profiles
    .filter(profile => isDiscoverableProfile(profile, currentUserId, needle))
    .slice(0, 30);
  return Promise.all(
    discoverableProfiles.map(async profile => {
      const subject = subjectFromOpenId(profile.openId);
      const request = currentSubject
        ? await readJson<StoredMessageRequest>(
            messageRequestPath(
              subject,
              directConversationId(currentSubject, subject)
            )
          )
        : undefined;
      const friendRequestStatus =
        currentSubject && request?.senderSubject === currentSubject
          ? request.status
          : null;
      ...
      return {
        ...
        subject,
        name: profile.name,
        username: profile.username ?? normalizedUsername(profile.name, subject),
        avatarStyle: storedProfile?.avatarStyle ?? "mint",
        profileImageUrl: await profileImageUrl(storedProfile),
        publicKey: profile.publicKey,
        isOnline: profile.isOnline,
...
export function isDiscoverableProfile(
  profile: Pick<User, "id" | "name" | "username" | "email" | "matricNumber">,
  ...
  return (
    !needle ||
    `${profile.name ?? ""} ${profile.username ?? ""} ${profile.email ?? ""} ${profile.matricNumber}`
      .toLowerCase()
      .includes(needle)
  );
}

async function updateProfileForUserId(
  userId: number,
  update: (profile: RemoteProfile) => RemoteProfile
) {
...
export async function setUserPresence(userId: number, online: boolean) {
  await updateProfileForUserId(userId, profile => ({
    ...profile,
    isOnline: online,
    lastSeenAt: now(),
    updatedAt: now(),
...
export async function getProfileSettings(userId: number) {
  ...
  const store = await getStore();
  const image =
    profile.profileImagePath && store
      ? await store.storage
          .from(BUCKET)
          .createSignedUrl(profile.profileImagePath, 60 * 15)
      : null;
  return {
    name: profile.name,
    username:
...
export async function updateProfileSettings(
  ...
  input: {
    ...
    imageType?: NonNullable<RemoteProfile["profileImageType"]> | null;
    clearImage?: boolean;
  }
) {
  ...
  const profile = await updateProfileForUserId(userId, current => ({
    ...
    profileImagePath:
      imagePath === undefined ? current.profileImagePath : imagePath,
    profileImageType:
      imageType === undefined ? current.profileImageType : imageType,
    updatedAt: now(),
  }));
  return {
    name: profile.name,
    ...
    profileImageUpdated: imagePath !== undefined,
  };
}
...
export async function getFriendProfile(userId: number, otherUserId: number) {
  ...
  const [viewerSubject, profile] = await Promise.all([
    subjectForUserId(userId),
  ...
  const relationship =
    ...
    conversation.participants.includes(viewerSubject)
      ? ("friends" as const)
      : request?.senderSubject === viewerSubject && request.status === "pending"
        ? ("pending" as const)
        : ("none" as const);
  return {
    id: profile.id,
...
export async function updatePrivacySettings(
  userId: number,
  input: { readReceiptsEnabled: boolean }
) {
  const profile = await updateProfileForUserId(userId, current => ({
    ...current,
...
async function isBlocked(subject: string, blockedSubject: string) {
  const record = await readJson<BlockRecord>(
    blockPath(subject, blockedSubject)
  );
  return (
    record?.subject === subject && record.blockedSubject === blockedSubject
  );
}

export async function isDirectContactBlocked(
  firstUserId: number,
  secondUserId: number
) {
  const [firstSubject, secondSubject] = await Promise.all([
    subjectForUserId(firstUserId),
    subjectForUserId(secondUserId),
  ]);
  return (
    (await isBlocked(firstSubject, secondSubject)) ||
    (await isBlocked(secondSubject, firstSubject))
  );
}

async function assertDirectContactAllowed(
  firstUserId: number,
  secondUserId: number
) {
  if (await isDirectContactBlocked(firstUserId, secondUserId))
    throw new Error("This contact is unavailable");
}

export async function listMessageRequests(userId: number) {
  const recipientSubject = await subjectForUserId(userId);
  const requests = await listJson<StoredMessageRequest>(
    `inboxes/${recipientSubject}/requests`
  );
  ...
  return Promise.all(
    ...
    items.filter(
      (item): item is { id: number; createdAt: Date; sender: User } =>
        Boolean(item.sender)
    )
  );
}

export function friendRequestResult(
  existingStatus?: StoredMessageRequest["status"]
) {
  return {
    status: "pending" as const,
    alreadyPending: existingStatus === "pending",
  };
}
...
export async function createMessageRequest(
  ...
  await assertDirectContactAllowed(userId, otherUserId);
  const [senderSubject, recipientSubject] = await Promise.all([
    subjectForUserId(userId),
    subjectForUserId(otherUserId),
  ]);
  const id = directConversationId(senderSubject, recipientSubject);
  const existing = await readJson<StoredMessageRequest>(
    messageRequestPath(recipientSubject, id)
  );
  if (existing?.status === "pending")
    return { requestId: id, ...friendRequestResult(existing.status) };
  const request: StoredMessageRequest = {
    id,
    senderSubject,
    recipientSubject,
    createdAt: now(),
...
export async function respondToMessageRequest(
  ...
  const recipientSubject = await subjectForUserId(userId);
  const request = await readJson<StoredMessageRequest>(
    messageRequestPath(recipientSubject, requestId)
  );
  if (
    !request ||
    request.recipientSubject !== recipientSubject ||
    request.status !== "pending"
  )
    throw new Error("Friend request is unavailable");
  const nextStatus = action === "accept" ? "accepted" : "declined";
  await writeJson(messageRequestPath(recipientSubject, requestId), {
    ...request,
    status: nextStatus,
  });
  ...
  return {
    conversationId: await getOrCreateDirectConversation(userId, senderId),
  };
}
...
export async function blockUser(userId: number, otherUserId: number) {
  ...
  const [subject, blockedSubject] = await Promise.all([
    subjectForUserId(userId),
    subjectForUserId(otherUserId),
  ]);
  await writeJson(blockPath(subject, blockedSubject), {
    subject,
    blockedSubject,
    createdAt: now(),
...
export async function unblockUser(userId: number, otherUserId: number) {
  ...
  const current = await readJson<BlockRecord>(
    blockPath(subject, blockedSubject)
  );
  if (current)
    await writeJson(blockPath(subject, blockedSubject), {
      ...current,
      blockedSubject: "released",
      subject: "released",
    });
}
...
export async function listBlockedUsers(userId: number) {
  ...
  return Promise.all(
    records
      .filter(
        record =>
          record.subject === subject && record.blockedSubject !== "released"
      )
      .map(async record => getUserByOpenId(`supabase:${record.blockedSubject}`))
  ).then(profiles =>
    profiles.filter((profile): profile is User => Boolean(profile))
  );
}
...
export async function setConversationPreference(
  userId: number,
  conversationId: number,
  preference: "pinned" | "muted" | "hidden",
  value: boolean
) {
  const subject = await subjectForUserId(userId);
  const current = await readJson<ConversationIndex>(
    conversationIndexPath(subject, conversationId)
  );
...
async function getConversationInfo(conversationId: number) {
  const conversation = await readJson<ConversationInfo>(
    conversationInfoPath(conversationId)
  );
  ...
  return {
    ...
    ownerSubject:
      conversation.ownerSubject ?? conversation.participants[0] ?? "",
    groupKeyVersion: conversation.groupKeyVersion ?? "v1",
    groupKeyEnvelopes: conversation.groupKeyEnvelopes ?? {},
    groupKeyEnvelopesByVersion: conversation.groupKeyEnvelopesByVersion ?? {
      [conversation.groupKeyVersion ?? "v1"]:
        conversation.groupKeyEnvelopes ?? {},
    },
  };
}
...
async function assertParticipant(userId: number, conversationId: number) {
  if (!(await userIsParticipant(userId, conversationId)))
...
async function upsertConversationIndex(
  subject: string,
  index: Omit<ConversationIndex, "pinned" | "muted" | "hidden"> &
    Partial<Pick<ConversationIndex, "pinned" | "muted" | "hidden">>
) {
  const existing = await readJson<ConversationIndex>(
    conversationIndexPath(subject, index.conversationId)
  );
  await writeConversationIndex(subject, {
    ...index,
...
export async function getOrCreateDirectConversation(
  ...
  if (!existing)
    await writeJson(conversationInfoPath(conversationId), conversation);
  await Promise.all([
    upsertConversationIndex(subject, {
      conversationId,
      peerSubject: otherSubject,
      kind: "direct",
      title: null,
    ...
    upsertConversationIndex(otherSubject, {
      conversationId,
      peerSubject: subject,
      kind: "direct",
      title: null,
...
export async function getAcceptedDirectConversation(
  userId: number,
  otherUserId: number
) {
  await assertDirectContactAllowed(userId, otherUserId);
  ...
  if (
    !conversation ||
    conversation.kind !== "direct" ||
    !conversation.participants.includes(subject) ||
    !conversation.participants.includes(otherSubject)
  )
    throw new Error("Accept the friend request before messaging");
  return conversationId;
}
...
export async function createGroupConversation(
  ...
  const participantSubjects = Array.from(
    new Set([
      ownerSubject,
      ...(await Promise.all(input.participantIds.map(subjectForUserId))),
    ])
  );
  if (participantSubjects.length < 3)
    throw new Error("A group needs at least three members");
  if (participantSubjects.length > 20)
    throw new Error("Groups are limited to 20 members");
  if (!input.title.trim()) throw new Error("Group name is required");
  if (participantSubjects.some(subject => !input.groupKeyEnvelopes[subject]))
    throw new Error("Each group member needs an encrypted key envelope");
  const id = messageId();
  const timestamp = now();
...
export async function addGroupParticipant(
  ...
  if (!conversation || conversation.kind !== "group")
    throw new Error("Group conversation was not found");
  if (conversation.ownerSubject !== subject)
  ...
  if (participants.length > 20)
    throw new Error("Groups are limited to 20 members");
  if (participants.some(participant => !input.groupKeyEnvelopes[participant]))
    throw new Error("Adding a member requires fresh encrypted key envelopes");
  const nextVersion = `v${Number(conversation.groupKeyVersion?.slice(1) ?? "1") + 1}`;
  const updated: ConversationInfo = {
    ...conversation,
    participants,
    groupKeyVersion: nextVersion,
    groupKeyEnvelopes: input.groupKeyEnvelopes,
    groupKeyEnvelopesByVersion: {
      ...(conversation.groupKeyEnvelopesByVersion ?? {
        [conversation.groupKeyVersion ?? "v1"]: conversation.groupKeyEnvelopes,
      }),
      [nextVersion]: input.groupKeyEnvelopes,
    },
    updatedAt: now(),
  };
...
export async function listGroupParticipants(
  ...
  const profiles = await Promise.all(
    conversation.participants.map(subject =>
      getUserByOpenId(`supabase:${subject}`)
    )
  );
  return profiles
...
export async function listConversations(userId: number) {
  const subject = await subjectForUserId(userId);
  const indexes = await listJson<ConversationIndex>(
    `inboxes/${subject}/conversations`
  );
  const items = await Promise.all(
    indexes
      .filter(index => !index.hidden)
      .map(async index => ({
        ...
        peer:
          index.kind === "direct"
            ? await getUserByOpenIdWithImage(`supabase:${index.peerSubject}`)
            : await getUserByOpenIdWithImage(`supabase:${index.peerSubject}`),
        groupKeyVersion:
          (await getConversationInfo(index.conversationId))?.groupKeyVersion ??
          "v1",
        groupKeyEnvelope:
          (await getConversationInfo(index.conversationId))?.groupKeyEnvelopes[
            subject
          ] ?? null,
        groupKeyEnvelopes: Object.fromEntries(
          Object.entries(
            (await getConversationInfo(index.conversationId))
              ?.groupKeyEnvelopesByVersion ?? {}
          ).map(([version, envelopes]) => [version, envelopes[subject] ?? null])
        ),
        latestMessageAt: asDate(index.latestMessageAt),
      }))
  );
  return items
    .filter(item => Boolean(item.peer))
    .sort(
      (a, b) =>
        Number(b.pinned) - Number(a.pinned) ||
        (b.latestMessageAt?.getTime() ?? 0) -
          (a.latestMessageAt?.getTime() ?? 0)
    );
}
...
export async function listEncryptedMessages(
  userId: number,
  conversationId: number
) {
  await assertParticipant(userId, conversationId);
  const messages = await listJson<StoredMessage>(
...
export async function uploadEncryptedAttachment(
  ...
  if (!bytes.length || bytes.length > MAX_ENCRYPTED_ATTACHMENT_BYTES)
    throw new Error("Encrypted attachment must be smaller than 3 MB");
  if (input.size < 1 || input.size > MAX_ENCRYPTED_ATTACHMENT_BYTES)
  ...
  return {
    ...
    name:
      ...
      (input.kind === "voice" ? "Voice note" : "Attachment"),
    mediaType:
      input.mediaType.trim().slice(0, 120) || "application/octet-stream",
    size: input.size,
    ciphertextPath,
    iv: input.iv,
    kind: input.kind,
...
export async function downloadEncryptedAttachment(
  userId: number,
  input: { conversationId: number; attachmentId: number }
) {
  await assertParticipant(userId, input.conversationId);
  const messages = await listJson<StoredMessage>(
  ...
  const attachment = messages.find(
    message => message.attachment?.id === input.attachmentId
  )?.attachment;
...
export async function createEncryptedMessage(input: {
  ...
  iv: string;
  keyVersion?: string;
  attachment?: NonNullable<StoredMessage["attachment"]> | null;
}) {
  ...
  if (conversation.kind === "direct") {
    const recipientSubject = conversation.participants.find(
      subject => subject !== senderSubject
    );
    if (!recipientSubject)
      throw new Error("Conversation recipient was not found");
  ...
  await Promise.all(
    conversation.participants.map(subject =>
      writeJson(messageIndexPath(subject, id), {
        conversationId: input.conversationId,
      })
    )
  );
  ...
  await Promise.all(
    recipients.map(async recipient => {
      const recipientIndex = await readJson<ConversationIndex>(
        conversationIndexPath(recipient, input.conversationId)
      );
      if (recipientIndex?.muted) return;
      const notification = notificationFor("new-message");
      const value: StoredNotification = {
        id,
        userSubject: recipient,
...
const DEVICE_LINK_TTL_MS = 3 * 60 * 1000;

function deviceLinkExpiry(expiresAt: string) {
...
export async function createDeviceLinkSession(
  ...
  const session: StoredDeviceLinkSession = {
  ...
  await writeJson(deviceLinkPath(session.sessionId), session);
  return {
...
export async function getDeviceLinkSession(userId: number, sessionId: string) {
  const subject = await subjectForUserId(userId);
  const session = await readJson<StoredDeviceLinkSession>(
    deviceLinkPath(sessionId)
  );
  if (
    !session ||
    (session.ownerSubject !== subject && session.recipientSubject !== subject)
  )
    throw new Error("Device link session was not found");
  if (!deviceLinkExpiry(session.expiresAt) || session.usedAt)
    return {
      sessionId,
      status: "expired" as const,
      expiresAt: session.expiresAt,
    };
  if (subject === session.ownerSubject) {
    return {
      sessionId,
      status: session.bundle
        ? ("completed" as const)
        : session.recipientPublicKey
          ? ("claimed" as const)
          : ("waiting" as const),
  ...
  return {
    sessionId,
    status: session.bundle ? ("completed" as const) : ("claimed" as const),
    bundle: session.bundle,
    expiresAt: session.expiresAt,
  };
}

export async function claimDeviceLinkSession(
  userId: number,
  sessionId: string,
  ...
  if (
    !session ||
    !deviceLinkExpiry(session.expiresAt) ||
    session.usedAt ||
    session.ownerSubject === recipientSubject
  )
    throw new Error("This device link is expired or unavailable");
  if (session.recipientSubject && session.recipientSubject !== recipientSubject)
    throw new Error("This device link has already been claimed");
  if (!session.recipientSubject) {
    await writeJson(deviceLinkPath(sessionId), {
      ...session,
      recipientSubject,
      recipientPublicKey,
    });
...
export async function completeDeviceLinkSession(
  userId: number,
  sessionId: string,
  bundle: StoredDeviceLinkBundle
) {
  const ownerSubject = await subjectForUserId(userId);
  const session = await readJson<StoredDeviceLinkSession>(
    deviceLinkPath(sessionId)
  ...
  if (
    ...
    throw new Error("This device link is expired or not ready");
  if (session.bundle) return { sessionId, expiresAt: session.expiresAt };
  await writeJson(deviceLinkPath(sessionId), { ...session, bundle });
  return { sessionId, expiresAt: session.expiresAt };
}

export async function consumeDeviceLinkSession(
  userId: number,
  sessionId: string
) {
  const recipientSubject = await subjectForUserId(userId);
  ...
  if (
    !session ||
    session.recipientSubject !== recipientSubject ||
    !session.bundle ||
    !deviceLinkExpiry(session.expiresAt) ||
    session.usedAt
  )
    throw new Error("This device link is expired or unavailable");
  await writeJson(deviceLinkPath(sessionId), { ...session, usedAt: now() });
...
export async function updateMessageStatus(
  ...
  const index = await readJson<{ conversationId: number }>(
    messageIndexPath(subject, messageIdValue)
  );
  if (!index) throw new Error("Message was not found");
  await assertParticipant(userId, index.conversationId);
  const message = await readJson<StoredMessage>(
    messagePath(index.conversationId, messageIdValue)
  );
  if (!message) throw new Error("Message was not found");
  if (message.senderId === userId) return;
  const nextStatus = advanceMessageStatus(
  ...
  const updated: StoredMessage = {
    ...
    deliveredAt:
      nextStatus === "delivered" || nextStatus === "read"
        ? timestamp
        : message.deliveredAt,
    readAt: nextStatus === "read" ? timestamp : message.readAt,
  };
...
function materializeNotification(notification: StoredNotification) {
  return {
    ...notification,
    userId: numericId(notification.userSubject),
...
export async function listNotifications(userId: number) {
  const subject = await subjectForUserId(userId);
  const notifications = await listJson<StoredNotification>(
    `inboxes/${subject}/notifications`
  );
  return notifications
    .sort((first, second) => second.createdAt.localeCompare(first.createdAt))
    .slice(0, 30)
    .map(materializeNotification);
}
...
export async function markNotificationRead(
  ...
  const subject = await subjectForUserId(userId);
  const notification = await readJson<StoredNotification>(
    notificationPath(subject, notificationId)
  );
  if (!notification || notification.userSubject !== subject) return;
  await writeJson(notificationPath(subject, notificationId), {
    ...notification,
    isRead: true,
  });
}
