...
export async function exportEncryptedRecoveryBundle(passphrase: string) {
  ...
  const ciphertext = await crypto.subtle.encrypt(
    ...
    new TextEncoder().encode(
      JSON.stringify({ version: RECOVERY_VERSION, identity })
    )
  );
...
export async function importEncryptedRecoveryBundle(
  serialized: string,
  passphrase: string
) {
  let bundle: RecoveryBundle;
  try {
    bundle = JSON.parse(serialized) as RecoveryBundle;
  } catch {
    throw new Error("Recovery file is not valid");
  }
  if (
    bundle.version !== RECOVERY_VERSION ||
    !bundle.salt ||
    !bundle.iv ||
    !bundle.ciphertext
  )
    throw new Error("Recovery file is not supported");
  try {
    ...
    throw new Error("Recovery passphrase or file is incorrect");
  }
}
...
export async function encryptIdentityForDevice(
  recipientPublicKey: JsonWebKey,
  senderKeyPair?: DeviceLinkKeyPair
): Promise<DeviceLinkBundle> {
  const identity = await getIdentity();
  const ephemeral = senderKeyPair ?? (await createDeviceLinkKeyPair());
  const key = await deviceLinkKey(ephemeral.privateKey, recipientPublicKey);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const ciphertext = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv },
    key,
    new TextEncoder().encode(
      JSON.stringify({ version: DEVICE_LINK_VERSION, identity })
    )
  );
  return {
...
export async function decryptIdentityFromDevice(
  privateKeyJwk: JsonWebKey,
  bundle: DeviceLinkBundle
) {
  if (
    bundle.version !== DEVICE_LINK_VERSION ||
    !bundle.senderPublicKey ||
    !bundle.iv ||
    !bundle.ciphertext
  )
    throw new Error("Device link is not supported");
  const key = await deviceLinkKey(privateKeyJwk, bundle.senderPublicKey);
  try {
    const plaintext = await crypto.subtle.decrypt(
    ...
    if (
      payload.version !== DEVICE_LINK_VERSION ||
      !payload.identity?.publicKey ||
      !payload.identity?.privateKey
    )
      throw new Error("Device link is not supported");
    ...
    throw new Error("This device link is invalid or expired");
  }
}
...
export async function prepareGroupKey(peerPublicKeys: Record<string, string>) {
  ...
  const envelopes = await Promise.all(
    Object.entries(peerPublicKeys).map(async ([subject, publicKey]) => {
...
export function saveGroupKey(
  conversationId: number,
  encodedKey: string,
  keyVersion = "v1"
) {
  localStorage.setItem(
    `${GROUP_KEY_PREFIX}${conversationId}.${keyVersion}`,
    encodedKey
  );
}

async function groupConversationKey(
  conversationId: number,
  keyVersion: string,
  envelope?: { ciphertext: string; iv: string; ownerPublicKey: string } | null
) {
  ...
  if (!envelope)
    throw new Error(
      "This device does not have the encrypted key for this group"
    );
...
async function conversationKey(conversationId: number, peerPublicKey: string) {
  ...
  const publicKey = await crypto.subtle.importKey(
    "jwk",
    JSON.parse(peerPublicKey) as JsonWebKey,
    { name: "ECDH", namedCurve: "P-256" },
    false,
    []
  );
...
export async function decryptGroupMessage(
  ...
  return new TextDecoder().decode(
    await decryptBytesWithKey(key, ciphertext, iv)
  );
}
...
export async function encryptAttachment(
  ...
  const key = groupEnvelope
    ? await groupConversationKey(conversationId, groupKeyVersion, groupEnvelope)
    : await conversationKey(conversationId, peerPublicKey);
...
export function isEncryptedPayload(value: string) {
  try {
    return fromBase64(value).length > 16;
  } catch {
    return false;
  }
}
