...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    secureChat: {
      ...
      requestMessage: {
        useMutation: () => ({
          ...
          mutateAsync: vi
            .fn()
            .mockImplementation(() =>
              Promise.resolve(directoryState.requestResult)
            ),
          isPending: false,
        }),
      },
      ...
      conversations: {
        useQuery: () => ({
          data: workspaceState.conversations,
          isLoading: false,
          error: null,
        }),
      },
      ...
      downloadEncryptedAttachment: {
        useQuery: () => ({ ...query(), refetch: vi.fn() }),
      },
      readNotification: { useMutation: mutation },
      createDeviceLinkSession: { useMutation: mutation },
      getDeviceLinkSession: {
        useQuery: () => ({ data: undefined, isLoading: false, error: null }),
      },
      claimDeviceLinkSession: { useMutation: mutation },
      completeDeviceLinkSession: { useMutation: mutation },
      consumeDeviceLinkSession: { useMutation: mutation },
    },
  },
}));
vi.mock("@/lib/crypto", () => ({
  createDeviceLinkKeyPair: vi.fn(),
  decryptIdentityFromDevice: vi.fn(),
  encryptIdentityForDevice: vi.fn(),
...
describe("Home authenticated handoff", () => {
  ...
  it("opens a profile menu without signing out until sign out is explicitly chosen", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Choose ink avatar" })
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "Choose mint avatar" })
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "Choose rose avatar" })
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "Choose violet avatar" })
    ).toBeTruthy();
    expect(screen.getByRole("button", { name: "Back to chat" })).toBeTruthy();
    expect(screen.getByText("Link a new device")).toBeTruthy();
  ...
  it("renders the mockup-aligned direct-chat structure for a selected conversation", async () => {
    workspaceState.conversations = [
      {
        ...
        peer: {
          ...
          name: "Elisha Onovo",
          username: "elisha",
          profileImageUrl: "https://cdn.example/elisha.jpg",
          isOnline: true,
          publicKey: "peer-key",
        },
        pinned: false,
    ...
    expect(screen.getByText("@elisha")).toBeTruthy();
    expect(
      screen.getAllByAltText("Elisha Onovo profile photo").length
    ).toBeGreaterThanOrEqual(1);
    expect(document.querySelector(".chat-shell")?.className).toContain(
      "max-w-full"
    );
    expect(document.querySelector(".chat-body")?.className).toContain(
      "overflow-x-hidden"
    );
    ...
    expect(
      screen.queryByText("Encrypted on this device before sending")
    ).toBeNull();
  });
