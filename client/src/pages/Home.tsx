...
export default function Home() {
  ...
  return <div className="min-h-screen blueprint-bg text-[#101722] overflow-hidden">
    ...
    <main className="h-[calc(100vh-4rem)] p-3 md:p-6 max-w-[1600px] mx-auto"><div className="h-full bg-white/75 border border-slate-900/15 shadow-[0_24px_80px_rgba(16,23,34,0.10)] flex overflow-hidden relative">
      ...
      <section className={mobileListOpen ? "hidden md:flex flex-1 min-w-0" : "flex flex-1 min-w-0"}>
        {activeConversation ? <div className="flex flex-1 min-h-0 flex-col bg-[#071523] text-white">
          <div className="shrink-0 border-b border-white/10 bg-[#071523] px-4 pb-5 pt-4 md:px-8 md:pb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-white hover:bg-white/10 hover:text-white md:hidden" onClick={() => setMobileListOpen(true)} aria-label="Back to conversations"><ChevronLeft className="h-5 w-5" /></Button>
                <SecureChatLogo size={30} />
                <span className="text-lg font-black tracking-tight">SecureChat</span>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" className="h-9 rounded-full border-white/20 bg-transparent px-3 text-[10px] font-mono uppercase tracking-wider text-white hover:bg-white/10 hover:text-white" onClick={() => setShowSecurity(value => !value)}><LockKeyhole className="mr-2 h-3.5 w-3.5" />Security</Button>
                <div className="relative">
                  <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-white hover:bg-white/10 hover:text-white" aria-label="Conversation actions" onClick={() => setShowActions(value => !value)}><MoreHorizontal className="h-5 w-5" /></Button>
            ...
            <button onClick={() => activeConversation.kind === "direct" && activeConversation.peer?.id && setFriendProfileUserId(activeConversation.peer.id)} className="mt-6 flex w-full items-center gap-4 text-left">
              <Avatar className="h-16 w-16 shrink-0 rounded-[1.35rem] border-2 border-white/10 bg-[#ffd1e1] text-lg">{activeConversation.peer?.profileImageUrl ? <img src={activeConversation.peer.profileImageUrl} alt="" className="h-full w-full rounded-[1.25rem] object-cover" /> : <AvatarFallback className="rounded-[1.25rem] bg-[#ffd1e1] font-black text-[#101722]">{initials(conversationName)}</AvatarFallback>}</Avatar>
              <span className="min-w-0"><span className="block truncate text-2xl font-black tracking-tight md:text-3xl">{conversationName}</span><span className="mt-1 block truncate text-sm text-white/55">{activeConversation.kind === "group" ? "Encrypted group" : <>{"@" + (activeConversation.peer?.username ?? "friend")} {activeConversation.peer?.isOnline && <span className="text-[#9ff1e4]">· online</span>}</>}</span></span>
            </button>
          </div>
          <ScrollArea className="min-h-0 flex-1 bg-[#071523]"><div className="mx-auto flex w-full max-w-3xl flex-col gap-7 px-4 py-7 md:px-8 md:py-10"><div className="flex items-center gap-3 text-[10px] font-mono uppercase tracking-[0.25em] text-[#f6a5c1]"><div className="h-px flex-1 bg-white/10" /><span>{activeConversation.kind === "group" ? "Group channel" : "Private chat"}</span><div className="h-px flex-1 bg-white/10" /></div>{messagesLoading ? <p className="py-12 text-center text-xs font-mono text-white/45">Decrypting local message history...</p> : messages.length ? messages.map(message => { const mine = message.senderId === user.id; const text = decrypted[message.id]; return <div key={message.id} className={"flex " + (mine ? "justify-end" : "justify-start")}><div className={"flex max-w-[88%] flex-col gap-2 " + (mine ? "items-end" : "items-start")}><div className={"px-5 py-4 text-[15px] leading-relaxed shadow-lg " + (mine ? "rounded-[1.6rem] rounded-br-md bg-[#b9f2e8] text-[#101722]" : "rounded-[1.6rem] rounded-bl-md bg-[#1c3045] text-white")}>{text || <span className={mine ? "text-[#101722]/45" : "text-white/45"}>Decrypting...</span>}{message.attachment && <EncryptedAttachmentCard conversation={activeConversation} attachment={message.attachment} keyVersion={message.keyVersion} />}</div><div className="flex items-center gap-2 px-1 text-[10px] font-mono uppercase tracking-wider text-white/45">{timeLabel(message.createdAt)} {mine && (message.readAt ? <CheckCheck className="h-3.5 w-3.5 text-[#9ff1e4]" /> : message.deliveredAt ? <CheckCheck className="h-3.5 w-3.5" /> : <Check className="h-3.5 w-3.5" />)}</div></div></div>; }) : <EmptyConversation peer={conversationName} />}</div></ScrollArea>
          <div className="shrink-0 border-t border-white/10 bg-[#071523] px-4 pb-4 pt-4 md:px-8 md:pb-7"><div className="mx-auto max-w-3xl"><div className="mb-3 flex items-center gap-2 text-sm font-medium text-[#f6a5c1]"><ShieldBan className="h-4 w-4" />Private chat</div><div className="flex items-end gap-2 rounded-[1.8rem] border border-white/20 bg-[#0d1c2d] p-2 shadow-2xl"><EncryptedMediaComposer conversation={activeConversation} onSent={() => refreshConversationData(selectedConversation ?? undefined)} /><Textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} placeholder="Write a message" className="min-h-[48px] flex-1 resize-none border-0 bg-transparent px-2 py-3 text-[15px] text-white placeholder:text-white/45 focus-visible:ring-0" /><Button onClick={() => void sendMessage()} disabled={!draft.trim() || sendMutation.isPending || !online} className="h-12 w-12 shrink-0 rounded-full bg-[#b9f2e8] text-[#101722] hover:bg-[#9ff1e4]" size="icon" aria-label="Send encrypted message">{sendMutation.isPending ? <span className="h-4 w-4 rounded-full border-2 border-[#101722]/35 border-t-[#101722] animate-spin" /> : <Send className="h-5 w-5" />}</Button></div><p className="mt-3 text-[10px] text-white/35">Encrypted on this device before sending</p></div></div>
        </div> : <NoActiveChat onStart={() => setMobileListOpen(true)} />}
      </section>
