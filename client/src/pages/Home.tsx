  ...
  return (
    <div className="min-h-screen blueprint-bg text-[#101722] overflow-hidden">
      <header className="h-16 border-b border-slate-900/10 bg-white/85 backdrop-blur-xl flex items-center justify-between px-4 md:px-8 relative z-20">
        <div className="flex items-center gap-4"><div className="h-9 w-9 bg-[#101722] text-white rounded-sm grid place-items-center font-mono font-bold">SC</div><p className="font-black tracking-tight text-lg leading-none">SecureChat</p></div>
        <div className="flex items-center gap-3"><div className="hidden sm:flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-slate-500"><span className={`h-2 w-2 rounded-full ${online && socketConnected ? "bg-emerald-500" : "bg-amber-500"}`} />{online && socketConnected ? "Connected" : "Reconnecting"}</div><Button variant="ghost" size="icon" className="rounded-sm relative" aria-label="Notifications" onClick={() => { setShowNotifications(value => !value); setShowProfileMenu(false); }}><MessageCircle className="h-4 w-4" />{unreadCount > 0 && <span className="absolute top-1 right-1 h-2 w-2 bg-[#ff4f87] rounded-full" />}</Button><div className="relative"><button onClick={() => { setShowProfileMenu(value => !value); setShowNotifications(false); }} className="flex items-center gap-2 group" aria-label="Open profile menu" aria-expanded={showProfileMenu}><Avatar className="h-8 w-8 rounded-sm"><AvatarFallback className="rounded-sm bg-[#c8f7f1] text-[#101722] font-mono text-xs">{initials(user.name)}</AvatarFallback></Avatar><span className="hidden md:block text-sm font-semibold group-hover:underline">{user.name ?? "University user"}</span></button>{showProfileMenu && <div className="absolute right-0 top-11 z-40 w-56 border border-slate-900/15 bg-white p-2 shadow-xl"><div className="px-3 py-2 border-b border-slate-900/10"><p className="text-sm font-semibold truncate">{user.name ?? "University user"}</p><p className="text-xs text-slate-500 truncate mt-0.5">{user.email ?? "Signed in"}</p></div><button onClick={() => void logout()} className="mt-1 flex w-full items-center rounded-sm px-3 py-2 text-left text-sm font-medium text-[#101722] hover:bg-[#ffd7e5]">Sign out</button></div>}</div></div>
      </header>
