...
function ProfileAvatar({ src, name, className, fallbackClassName, accent }: { src?: string | null; name?: string | null; className: string; fallbackClassName?: string; accent?: string }) {
  ...
  return <Avatar className={className}>{src && !imageFailed ? <img src={src} alt={`${name ?? "Friend"} profile photo`} className="h-full w-full object-cover" onError={() => setImageFailed(true)} /> : <AvatarFallback className={fallbackClassName} style={accent ? { background: accent } : undefined}>{initials(name)}</AvatarFallback>}</Avatar>;
}
  ...
  return <div className="min-h-screen blueprint-bg text-[#101722] overflow-hidden">
    ...
    <main className="h-[calc(100vh-4rem)] p-3 md:p-6 max-w-[1600px] mx-auto"><div className="h-full bg-white/75 border border-slate-900/15 shadow-[0_24px_80px_rgba(16,23,34,0.10)] flex overflow-hidden relative">
      ...
      <section className={mobileListOpen ? "hidden md:flex flex-1 min-w-0" : "flex flex-1 min-w-0"}>
        {activeConversation ? <div className="chat-shell flex flex-1 min-h-0 flex-col">
          <div className="chat-header shrink-0 border-b px-4 pb-5 pt-4 md:px-8 md:pb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Button variant="ghost" size="icon" className="chat-header-action h-9 w-9 rounded-full md:hidden" onClick={() => setMobileListOpen(true)} aria-label="Back to conversations"><ChevronLeft className="h-5 w-5" /></Button>
          ...
          <ScrollArea className="chat-body min-h-0 flex-1"><div className="mx-auto flex w-full max-w-3xl flex-col gap-7 px-4 py-7 md:px-8 md:py-10"><div className="flex items-center gap-3 text-[10px] font-mono uppercase tracking-[0.25em] text-[#d77d9e]"><div className="chat-divider h-px flex-1" /><span>{activeConversation.kind === "group" ? "Group channel" : "Private chat"}</span><div className="chat-divider h-px flex-1" /></div>{messagesLoading ? <p className="chat-meta py-12 text-center text-xs font-mono">Decrypting local message history...</p> : messages.length ? messages.map(message => { const mine = message.senderId === user.id; const text = decrypted[message.id]; return <div key={message.id} className={"flex " + (mine ? "justify-end" : "justify-start")}><div className={"flex max-w-[88%] flex-col gap-2 " + (mine ? "items-end" : "items-start")}><div className={(mine ? "chat-bubble-outgoing rounded-[1.6rem] rounded-br-md" : "chat-bubble-incoming rounded-[1.6rem] rounded-bl-md") + " px-5 py-4 text-[15px] leading-relaxed shadow-lg"}>{text || <span className="chat-meta">Decrypting...</span>}{message.attachment && <EncryptedAttachmentCard conversation={activeConversation} attachment={message.attachment} keyVersion={message.keyVersion} />}</div><div className="chat-meta flex items-center gap-2 px-1 text-[10px] font-mono uppercase tracking-wider">{timeLabel(message.createdAt)} {mine && (message.readAt ? <CheckCheck className="h-3.5 w-3.5 text-[#70b6aa]" /> : message.deliveredAt ? <CheckCheck className="h-3.5 w-3.5" /> : <Check className="h-3.5 w-3.5" />)}</div></div></div>; }) : <EmptyConversation peer={conversationName} />}</div></ScrollArea>
          <div className="chat-composer shrink-0 border-t px-4 pb-4 pt-4 md:px-8 md:pb-7"><div className="mx-auto max-w-3xl"><div className="mb-3 flex items-center gap-2 text-sm font-medium text-[#d77d9e]"><LockKeyhole className="h-4 w-4" />Private chat</div><div className="chat-composer-box flex items-end gap-2 rounded-[1.8rem] border p-2 shadow-lg"><EncryptedMediaComposer conversation={activeConversation} onSent={() => refreshConversationData(selectedConversation ?? undefined)} /><Textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} placeholder="Write a message" className="min-h-[48px] flex-1 resize-none border-0 bg-transparent px-2 py-3 text-[15px] text-[#101722] placeholder:text-slate-500 focus-visible:ring-0" /><Button onClick={() => void sendMessage()} disabled={!draft.trim() || sendMutation.isPending || !online} className="h-12 w-12 shrink-0 rounded-full bg-[#b9f2e8] text-[#101722] hover:bg-[#9ff1e4]" size="icon" aria-label="Send encrypted message">{sendMutation.isPending ? <span className="h-4 w-4 rounded-full border-2 border-[#101722]/35 border-t-[#101722] animate-spin" /> : <Send className="h-5 w-5" />}</Button></div></div></div>
