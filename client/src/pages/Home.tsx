...
export default function Home() {
  ...
  const chooseUser = async (userId: number) => {
    try {
      await requestMutation.mutateAsync({ userId });
      setSearch("");
      setPeopleOpen(false);
  ...
  return <div className="min-h-screen blueprint-bg text-[#101722] overflow-hidden">
    <header className="h-16 border-b border-slate-900/10 bg-white/85 backdrop-blur-xl flex items-center justify-between px-4 md:px-8 relative z-20"><div className="flex items-center gap-4"><div className="h-9 w-9 bg-[#101722] text-white rounded-sm grid place-items-center font-mono font-bold">SC</div><p className="font-black tracking-tight text-lg leading-none">SecureChat</p></div><div className="flex items-center gap-3"><div className="hidden sm:flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-slate-500"><span className={`h-2 w-2 rounded-full ${online && socketConnected ? "bg-emerald-500" : "bg-amber-500"}`} />{online && socketConnected ? "Connected" : "Reconnecting"}</div><Button variant="ghost" size="icon" className="rounded-sm" aria-label={theme === "dark" ? "Use light mode" : "Use dark mode"} onClick={toggleTheme}>{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button><Button variant="ghost" size="icon" className="rounded-sm relative" aria-label="Notifications" onClick={() => setShowNotifications(value => !value)}><MessageCircle className="h-4 w-4" />{unreadCount > 0 && <span className="absolute top-1 right-1 h-2 w-2 bg-[#ff4f87] rounded-full" />}</Button><ProfileControls user={user} onSignOut={logout} /></div></header>
    ...
    <main className="h-[calc(100vh-4rem)] p-3 md:p-6 max-w-[1600px] mx-auto"><div className="h-full bg-white/75 border border-slate-900/15 shadow-[0_24px_80px_rgba(16,23,34,0.10)] flex overflow-hidden relative">
      <aside className={`${mobileListOpen ? "flex" : "hidden"} md:flex w-full md:w-[340px] shrink-0 border-r border-slate-900/10 flex-col bg-white/80`}><div className="p-5 border-b border-slate-900/10"><div className="flex items-start justify-between mb-5"><div><p className="font-mono text-[10px] text-slate-400 uppercase tracking-[0.24em]">Inbox</p><h1 className="text-3xl font-black tracking-[-0.05em] mt-1">Messages</h1></div><Badge className="rounded-sm bg-[#ffd7e5] text-[#101722] hover:bg-[#ffd7e5] font-mono text-[10px]">ENCRYPTED</Badge></div><div className="flex gap-2 mb-3"><GroupCreator currentUser={user} onCreated={(id) => { setSelectedConversation(id); setMobileListOpen(false); void utils.secureChat.conversations.invalidate(); }} /><Button variant="outline" onClick={() => { setPeopleOpen(value => !value); setSearch(""); }} className="rounded-sm h-11 font-mono text-[10px] uppercase tracking-wider"><UsersRound className="h-3.5 w-3.5 mr-2" />People</Button></div><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><Input value={search} onChange={event => { setSearch(event.target.value); setPeopleOpen(false); }} placeholder="Find a student" className="h-10 rounded-sm border-slate-900/15 bg-slate-50 pl-10 font-mono text-xs" /></div></div>
      {(search.length > 1 || peopleOpen) && <div className="border-b border-slate-900/10 p-2 space-y-1">{peopleOpen && <div className="flex items-center justify-between px-3 pt-2 pb-1"><div><p className="text-sm font-bold">People</p><p className="text-[11px] text-slate-500">Other registered students</p></div><button onClick={() => setPeopleOpen(false)} aria-label="Close people" className="p-1 text-slate-500 hover:text-[#101722]"><X className="h-4 w-4" /></button></div>}{results.length ? results.map((person, index) => <button key={person.id} onClick={() => void chooseUser(person.id)} className="w-full flex items-center gap-3 p-3 text-left hover:bg-[#c8f7f1]/45 transition-colors"><Avatar className="h-9 w-9 rounded-sm"><AvatarFallback className="rounded-sm" style={{ background: accents[index % accents.length] }}>{initials(person.name)}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="text-sm font-semibold truncate">{person.name || "Unnamed user"}</p><p className="text-[11px] text-slate-500 truncate">{person.email || "Registered account"}</p></div><span className="font-mono text-[9px] uppercase tracking-widest text-slate-400"><UserRoundPlus className="inline h-3 w-3 mr-1" />Request</span></button>) : <p className="p-3 text-xs text-slate-500 font-mono">No other registered students found yet.</p>}</div>}
...
function NoActiveChat({ onStart }: { onStart: () => void }) { return <div className="flex-1 grid place-items-center p-10"><div className="max-w-md text-center"><div className="h-20 w-20 mx-auto border border-slate-900/15 bg-[#c8f7f1] grid place-items-center rotate-[-6deg]"><MessageCircle className="h-8 w-8" /></div><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-slate-400 mt-8">SecureChat</p><h2 className="text-4xl font-black tracking-[-0.06em] mt-2">Choose a conversation.</h2><p className="text-slate-500 mt-4 leading-relaxed">Start with a message request or create a secure group for people you know.</p><Button onClick={onStart} variant="outline" className="rounded-sm mt-7 md:hidden">Open conversations</Button></div></div>; }
